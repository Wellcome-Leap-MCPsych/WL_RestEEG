...
% EEGLAB 
% The following extensions/plugins
% dipfit, firfilt, ERPLAB, FASTER, ICLabel, loadcurry
...
for subjID = 1:length(rawData) 
    ...
    EEG = loadcurry(strcat(rawData(subjID).folder,'\',rawData(subjID).name), 'CurryLocations', 'True'); % Load data for UCSD team
    ...
    EEG = pop_select(EEG,'nochannel',{'VEOG';'HEOG';'TRIGGER';'EMG';'EKG'}'); % UCSD
    ...
    % Step 4: Remove bad channels - FASTER pipeline
    EEG = pop_reref(EEG, {'Pz'}, 'keepref', 'on');       % Temporarily re-reference to Pz; required for FASTER

    list_properties = channel_properties(EEG, 1:EEG.nbchan, 50);     % Computes bad channel coefficents; Arguments are: Data input, Which Chans, Ref Chan ID   
    badChanindices = min_z(list_properties);
    badChans = find(badChanindices == 1);   % Identifies bad channels
    badChans = badChans(badChans ~= 50);     % Removes the reference from list so it's not excluded   					

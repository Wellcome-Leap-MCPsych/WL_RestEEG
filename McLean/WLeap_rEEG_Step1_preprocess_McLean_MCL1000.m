...
cd('C:/Users/tlees/Dropbox (Partners HealthCare)/McLean 1000 EEG Data/Lees_RestingState_Preprocessing/EO_Data/');     %Set your CD to where you keep the data; this will work with cloud directories if you have the desktop apps

%% Load raw files and concatenate together
rawData = dir('*_EO*.vhdr');    % Search for all files for one Px ID
...
for i = 1:4:length(rawData)   
    for j = 0:3
        ...
        if (rawData(i).name(4:6) == rawData(i+1).name(4:6)) & (rawData(i).name(4:6) == rawData(i+2).name(4:6)) & (rawData(i).name(4:6) == rawData(i+3).name(4:6)) & ...
                (rawData(i+1).name(4:6) == rawData(i+2).name(4:6)) & (rawData(i+1).name(4:6) == rawData(i+3).name(4:6)) & ...
                (rawData(i+2).name(4:6) == rawData(i+3).name(4:6))
    ...
    pop_saveset(ALLEEG(end), 'filename', strcat(rawData(i).folder,'\Merged Data\',rawData(i).name(1:end-6),'_merged.set'), ...
...
Stanford = pop_loadset('Stanford_locs.set',...
                   'C:\Users\tlees\Dropbox (Partners HealthCare)\Postdoc - McLean\Research Projects\WellcomeLEAP_personal\McLean Test Data\');
...
for subjID = 1:length(mergedData) 
    ...
    % Step 7: ICA
        ...
        EEG = pop_subcomp(EEG, [], 0);  % Remov	e components flagged by ICLabel
...
writematrix(FilesProcessed, strcat(cd,'\WL_PAM_FilesProcessed_', string(date),'.txt'))
